        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <select id="tipoPrestamo" name="tipo_prestamo" class="form-select @error('tipo_prestamo') is-invalid @enderror">
                            ...
                            @foreach($tipos as $tipo)
                                <option
                                    value="{{ $tipo->id_tasa }}"
                                   @selected(old('tipo_prestamo', $prestamo->tipo_prestamo ?? '') == $tipo->id_tasa)
                    ...
                    <div class="col-md-5">
                        <x-scas.papeleta-search name="id_socio" label="Solicitante" :value="old('id_socio', $prestamo->socio->id ?? '')" />
                    </div>
                     <div class="col-md-3">
                        <label class="form-label">Fecha Depósito</label>
                        <input type="date" id="fechaPrestamo" name="fechaPrestamo" class="form-control @error('fecha') is-invalid @enderror"
                        value="{{ old('fechaPrestamo',
                            isset($prestamo)
                                ? \Carbon\Carbon::parse($prestamo->fecha_deposito)->format('Y-m-d')
                                : now()->format('Y-m-d')
                        ) }}">
